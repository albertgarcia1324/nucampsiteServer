    ...
    Campsite.findById(req.params.campsiteId)
      .then((campsite) => {
        if (campsite) {
          ...
          campsite.comments.unshift(req.body);
          campsite
            .save()
            .then((campsite) => {
              res.statusCode = 200;
      ...
      .then((campsite) => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
          if (
            ...
            err = new Error("Only author is allowed to edit this comment");
            err.status = 403;
            return next(err);
          }
        } else if (!campsite) {
      ...
      .then((campsite) => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
          if (
            ...
            err = new Error("Only author is allowed to delete this comment");
            err.status = 403;
            return next(err);
          }
        } else if (!campsite) {
